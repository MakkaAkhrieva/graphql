...
function createMemberLoader(prisma: PrismaClient) {
  return new DataLoader<unknown, Member | null>(async (userIds) => {
...
function createPostsLoader(prisma: PrismaClient) {
  return new DataLoader<unknown, Post[]>(async (userIds) => {
    const ids = userIds as string[];
...
function createProfileLoader(prisma: PrismaClient) {
  return new DataLoader<unknown, Profile | null>(async (userIds) => {
    const ids = userIds as string[];
...
function createSubscribedToUserLoader(prisma: PrismaClient) {
  return new DataLoader<string, Author[]>(async (userIds) => {
    const ids = userIds as string[];
...
function createUserToSubscribeLoader(prisma: PrismaClient) {
  return new DataLoader<string, Subscription[]>(async (userIds) => {
    const ids = userIds as string[];
